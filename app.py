...
def load_model():
    ...
    try:
        ...
        # ── Use timm's own config so mean/std/size exactly match training ──
        data_cfg = resolve_data_config({}, model=model)
        # Override to match training script exactly
...
def predict_real(img: Image.Image, model, tfm, class_names, top_k=5):
    import torch
    img_rgb = img.convert("RGB")          # handles RGBA, palette, greyscale
    x = tfm(img_rgb).unsqueeze(0)
    with torch.no_grad():
        logits = model(x)[0]
        probs  = torch.softmax(logits, dim=0).numpy()
    top_idx = np.argsort(probs)[::-1][:top_k]
    # Store diagnostics in session state for debug panel
    import streamlit as st
...
with col_upload:
    ...
    if all_samples:
        # Group samples by crop (first token before __)
        from collections import defaultdict
        grouped = defaultdict(list)
        for sp in all_samples:
            # Filename pattern: ClassName__N.jpg  e.g. Apple__Apple_scab__1.jpg
            crop = sp.stem.split("__")[0].replace("_", " ").split("(")[0].strip()
            grouped[crop].append(sp)

        # Crop selector dropdown
        crop_options = sorted(grouped.keys())
        selected_crop = st.selectbox(
            "Filter by crop",
            ["All crops"] + crop_options,
            key="crop_filter",
            label_visibility="collapsed",
        )
        ...
        if selected_crop == "All crops":
            ...
            visible = grouped[selected_crop]

        # Show thumbnails in a 3-column grid (max 12 at a time)
        visible = visible[:12]
        n_cols  = 3
        ...
        for row in rows:
            ...
            for col, sp in zip(cols, row):
                with col:
                    # Clean label: "Apple Scab" from "Apple__Apple_scab__1"
                    parts = sp.stem.split("__")
                    if len(parts) >= 2:
                        label = parts[1].replace("_", " ").title()
                        # strip trailing digit if any
                        label = label.rstrip("0123456789").strip()
                    else:
                        label = sp.stem.replace("_", " ")
                    label = label[:16]   # truncate for button width

                    try:
                        thumb = Image.open(sp).convert("RGB").resize((90, 90))
        ...
        st.info(
            "📁 Run `python setup_samples.py` to populate real sample images, "
...
with col_result:
    ...
    if img_pil is None:
        ...
        with st.expander("🛠 Debug — raw model output (click to inspect)", expanded=False):
            # Logit health
            lmax = st.session_state.get("_dbg_logit_max", None)
            lmin = st.session_state.get("_dbg_logit_min", None)
            t1p  = st.session_state.get("_dbg_top1_prob", None)
            if lmax is not None:
                healthy = (lmax - lmin) > 3.0 and t1p > 0.5
                verdict = "✅ Model output looks healthy" if healthy else \
                          "⚠️ LOW CONFIDENCE — model is confused. Use a real leaf photo."
                st.markdown(f"**Logit range:** {lmin:.2f} → {lmax:.2f} &nbsp;|&nbsp; "
                            f"**Top-1 prob:** {t1p*100:.1f}% &nbsp;|&nbsp; {verdict}")
            st.markdown("**Raw class names from model → matched JSON key:**")
            for rc, rc_conf in preds:
                _, mk = lookup_disease(rc, disease_info)
                match_ok = "✅" if rc in disease_info else "⚠️ fuzzy"
                st.code(f"{rc_conf*100:5.1f}%  |  model: {rc!r}\n"
                        f"              mapped: {mk!r}  {match_ok}")
